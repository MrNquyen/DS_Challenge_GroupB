...
class CLIPTextEmbeddings(nn.Module):
    ...
    def forward(
        ...
        # print(f'Interactive Clip - inputs_embeds {inputs_embeds.shape} - position_embeddings {position_embeddings.shape}')
...
class CLIPAttention(nn.Module):
    ...
    def forward(
        ...
        if conditional_hidden_states is not None:
            adapter_proj_conditional_hidden_states = self.adapter_proj(conditional_hidden_states)
            # print(f'hidden_states: {hidden_states.shape} - adapter_proj_conditional_hidden_states: {adapter_proj_conditional_hidden_states.shape}')
            hidden_states = (
                torch.cat(
...
class CLIPEncoderLayer(nn.Module):
    def __init__(self, config: CLIPConfig, is_add_fusion: bool = False):
        super().__init__()
        # print(f'Line 318 CLIPEncoderLayer')
        self.embed_dim = config.hidden_size
...
class CLIPEncoder(nn.Module):
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            # print(f'Line 540: InteractiveCLIP: conditional_hidden_states: {type(conditional_hidden_states)}')
            args = (
                (
                    hidden_states,
